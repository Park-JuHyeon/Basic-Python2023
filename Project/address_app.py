...
# 15 - 
# 1 파일 없을때 나는 예외
# 15 - 2 입력시 / 개수가 다를 때의 예외
# 15 - 3 메뉴번호 입력 숫자가 아닐때 예외
import os    # 운영체제용 모듈
# 2. 클래스생성
class Contact:
    # 생성자 - 이름, 전번, 이메일, 주소
...
# 5. 사용자입력
def set_contact():    # set은 입력
    name, phone_num, email, addr = input('정보 입력(이름/ 전번/ 이메일/ 주소) > ').split('/')
...
def save_contacts(list):
    file = open('C:/Source/study-Python2023/Project/contacts.txt', 'w',
                 encoding='utf-8')
    for item in list:
        text = f'{item.getName()}/{item.getPhoneNum()}/{item.getEmail()}/{item.getAddr()}'
        file.write(f'{text}\n')

    file.close()  # 파일은 반드시 마지막에 닫아줘야한다.!!

# 14. 주소록 읽어오기
def load_contacts(list):
    try:
        file = open('C:/Source/study-Python2023/Project/contacts.txt', 'r',
                 encoding='utf-8')
    except Exception as e:     # 15 - 1 예외처리
        f = open('C:/Source/study-Python2023/Project/contacts.txt', 'w',
                 encoding='utf-8')
        f.close()  # 파일이 없어서 생기는 예외는 파일생성하고 함수를 탈출함
        return
...
# 추가. 화면클리어 (항상 메뉴의 위치를 맨위로 만들어줌.)
def clear_console():
    command = 'clear'  # Linux, Unix 화면 클리어 명령어
    if os.name in ('nt','dos'):   # window 운영체제이면 
...
def run():
    ...
    # temp = Contact('박주현','010-8337-6696','wngus2tp@naver.com',
    #                 '부산광역시 사하구...')
    # set_contact()
    clear_console()
    while True:
